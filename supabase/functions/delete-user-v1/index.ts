...
/**
 * The `delete-user-v1` edge function is used to delete the current user. When the user is deleted all the corresponding
 * user data is also removed, so that we should warn a user about this action.
 */
serve(async (req) => {
  /**
  ...
  try {
     ...
     * is required because the user client does not have the permissions to delete a user.
     */
    ...
    /**
     * Before the user can be deleted we have to get the users profile from the database to check if the user has an
     * active Stripe subscription. If this is the case we return an error, so that we do not charge a user for his
     * active subscription while the account is already deleted.
     */
    ...
    /**
     * Delete the current user via the admin client. If there is an error, we return an error. If the user was deleted
     * successfully we return a 204 response.
     */
