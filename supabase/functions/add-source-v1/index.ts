...
/**
 * The `add-source-v1` edge function is used to add a new source and it's corresponding items to the database. It
 * expects a POST request with the column id, source type and options. The function will return the source object as it
...
serve(async (req) => {
  ...
  try {
     ...
     * is required because the user client does not have the permissions to access the `profiles` table or to insert new
     * row in the `sources` and `items` table.
     */
    ...
    /**
     * Get the number of sources the user has created. If there is an error, we return an error.
     *
     * Based on the number of sources and the user `tier` from the profiles table from the former step, we decide if the
     * user can create a new source. If a user is on the free `tier`, they can create up to 10 sources. If a user is on
     * the `premium` tier, they can create up to 1000 sources.
     */
    ...
    /**
     * Get the column, source typ and options from the request. Based on this information, we get the source and items
     * from the `getFeed` function.
     */
    ...
    /**
     * Save the `sources` and `items` to our database. The `items` are only save if there is at least one item in the
     * array. If there are no items, we only save the source.
     *
     * If there is an error, we return an error. If the insert succeeds we return the source object.
     */
