...
serve(async (req) => {
  ...
  try {
    ...
    /**
     * When the request method is GET, we return the user profile. The profile contains the users tier ("free" or
     * "premium") and the users connected accounts. We do not return the users account values directly instead we
     * convert them to `true` or `false`, which indicates if the account is connected or not.
     */
    if (req.method === "GET") {
    ...
    /**
     * When the request method is POST, we try to connected a users Social Media account with the users profile. The
     * account type is defined in the request body via the `sourceType` property. Depending on the `action` property we
     * either add a new account or initialize the connection process, e.g. setting a verify token and returning an url
     * for authentication.
     */
    if (req.method === "POST") {
      const data: {
      ...
      /**
       * If the request data contains the `action` property with the value `add-account` and the `sourceType` property
       * with the value `github` and the `options` property with the `token` property, we add the GitHub account to the
       * users profile.
       */
      if (
      ...
      /**
       * If the request data contains the `action` property with the value `delete-account` and the `sourceType`
       * property is `github`, we delete the users GitHub account from his profile by setting the value of the
      ...
      /**
       * If the request data doesn't match any of the above conditions, we return an error.
       */
